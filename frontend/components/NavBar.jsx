...
export default function NavBar() {
  ...
  const [mobileOpen, setMobileOpen] = useState({ products: false, tools: false }); // mobile accordions
      ...
      >
        <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-slate-200">
          <div className="flex items-center gap-2">
            <Image src="/logo_dark.png" alt="Logo" width={22} height={22} className="rounded" />
            <span className="text-sm font-semibold">Browse, compare, calculate — all here</span>
          </div>
          <button
        ...
        <nav className="px-4 py-4 text-[15px] space-y-2">
          ...
          <div className="rounded-xl bg-slate-50/60">
            ...
            >
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" className={`transition ${mobileOpen.products ? "rotate-180" : ""}`}>
            ...
            >
              <div className="min-h-0">
                {NAV.find((n) => n.id === "products").children.map((c) => (
                  <Link key={c.href} href={c.href} onClick={closeDrawer} className="block px-4 py-2 text-slate-700 hover:bg-white">
                    {c.label}
                  </Link>
                ))}
              </div>
          ...
          <div className="rounded-xl bg-slate-50/60">
            ...
            >
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" className={`transition ${mobileOpen.tools ? "rotate-180" : ""}`}>
                <path d="M6 9l6 6 6-6" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
              </svg>
            </button>
            <div
